...
class hashEA {
    ...
    public:
        ...
        // Aumenta a capacidade do vetor de dados
        void aumenta();
};


void hashEA::aumenta() {
	unsigned capAntiga = capacidade;
	capacidade *= 1.5;
	dado* novoVet = new dado[capacidade];
	unsigned pos;
	for (unsigned i = 0; i < capAntiga; i++) {
		if (vetDados[i] != INVALIDO) {
			pos = posicao(vetDados[i].nomeInteressado);
			while ((novoVet[pos] != INVALIDO) and (novoVet[pos] != REMOVIDO))
				pos = (pos + 1) % capacidade;
			novoVet[pos].assunto = vetDados[i].assunto;
			novoVet[pos].nomeInteressado = vetDados[i].nomeInteressado;
			novoVet[pos].tipo = vetDados[i].tipo;
			novoVet[pos].numeroProcesso = vetDados[i].numeroProcesso;
		}
	}
	delete [] vetDados;
	vetDados = novoVet;
...
//		thrown runtime_error("Tabela hash cheia.");
