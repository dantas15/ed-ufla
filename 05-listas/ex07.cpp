...
public:
    ...
    void insereOrdenado(acaoPrograma acao);
...
void listadup::insereOrdenado(acaoPrograma acao) {
	noh* novo = new noh(acao);

	if (this->vazia() || acao.identificador < this->primeiro->acao.identificador) {
		this->insereNoInicio(acao);
		return;
	} else if (acao.identificador > this->ultimo->acao.identificador) {
		this->insereNoFim(acao);
	 } else {
		noh* aux = this->primeiro;
		while (aux->proximo != NULL && aux->proximo->acao.identificador < acao.identificador) {
			aux = aux->proximo;
		}
		novo->anterior = aux;
        ...
        try {
            ...
            switch (comando) {
                case 'i': // inserir
                    ...
                    minhaLista.insereOrdenado(info);
                    break;
                case 'h': // inserir
                    cin >> info.identificador >> info.nomeAcao >> info.tempoExecucao >> info.tempoConsumido;
                    minhaLista.insereOrdenado(info);
